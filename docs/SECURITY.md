...
| Surface                  | Description                                                                                                                                                                                                                       |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tauri IPC                | `invoke()` commands from the React UI to the Rust shell. Used for FS operations, OS-level commands, and Epic 03 project commands. The IPC channel is local-process-only — not accessible from outside the app window.            |
| HTTP loopback (internal) | `GET /health` issued by the Tauri shell to the sidecar. Bound to `127.0.0.1` (not `0.0.0.0`); not reachable from other machines.                                                                                                 |
| HTTP loopback (UI direct) | `GET /assets` and `GET /events` (SSE) issued directly by the React UI to the sidecar, per ADR-009. Restricted to read-only queries; bound to `127.0.0.1` only. CORS headers are set to `Access-Control-Allow-Origin: *` — this is safe because the loopback binding is the security boundary. Any other local process on the same machine that discovers the ephemeral port can issue these read-only requests; see Known Limitations. |
...
- **HTTP loopback is not authenticated.** Any local process on the same machine that discovers the sidecar's ephemeral port can issue requests to `GET /health`, `GET /assets`, and `GET /events`. All three endpoints are read-only (no mutation, no secrets exposed). The `GET /assets` response reflects the current SQLite asset index derived from `~/.claude/`; this is no more sensitive than what a local process could read from `~/.claude/` directly. Future endpoints that add mutation operations (e.g., Epics 07–09) must be audited for this exposure before shipping.

---

_Last reviewed: 2026-05-20 by tech-writer agent against Sprint 5 (ADR-009 direct-fetch path added to surface area)._
